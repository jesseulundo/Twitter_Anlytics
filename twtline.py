...
result = cur.execute("SELECT message FROM tweets GROUP BY date")
...
most_common = count_all.most_common(10)
sorted(most_common,key=lambda x:x[1])
most_common = dict(most_common)

print("MOST POPULAR TERMS: ")
print(most_common)
...
for dates in cur:
	for days in dates:
		...
		d = day[0:3]
		dias = ' '.join(map(str, d))
		if dias not in days_time : days_time.append(dias)

days_time.sort()
fhand = open('twtline.js','w', encoding='utf-8')
fhand.write("twtline = [ ['Days'")
for palavras in x:
	fhand.write(",'"+str(x)+"'")
fhand.write("]")

for dia in days_time:
	fhand.write(",\n['"+dia+"'")
	for k in x:
		key = (dia, k)
...
print("Output written to twtline.js")
print("Open twtline.htm to visualize the data")
